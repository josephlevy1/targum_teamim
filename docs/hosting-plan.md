...
Cost-safety policy: remain within free-tier allowances. Configure usage alerts where they are useful and enforce an application-side R2 storage cutoff at 9 GB; if projected usage would exceed that threshold, stop cloud uploads and serve an availability/error notice rather than incur an overage. Keep source files and recovery backups on the Mac mini. The runtime cutoff is implemented through `R2_STORAGE_CUTOFF_BYTES`, which defaults to 9 GiB.
